...
| Neo4j export | `examples/run-example.js` |
...
Matrices are generated by the extension at build time and registered as attachments. `run-example.js` exports the Neo4j CSV (`nodes.csv`, `relationships.csv`).
...
- **Regenerate and verify** — always rebuild after document changes, then check for xref errors (matrices are generated by the extension at build time).
