...
[item, id=ARC-001, role=architecture, title="Project scope and architectural goals"]
--
...
[item, id=ARC-002, role=architecture, title="Component-level architecture"]
--
...
[item, id=ARC-003, role=architecture, title="Processing flow during Antora build"]
--
...
== Pass 2: Substitute links ==
loop for each .adoc file
  antora_ext -> antora_ext : substituteRelationshipLinks(content)
  antora_ext -> graph : getItem(targetId) for each macro match
  graph --> antora_ext : target item (sourceFile)
  antora_ext -> antora_ext : replace macro with xref#ID or xref:page.adoc#ID
...
[item, id=ARC-004, role=architecture, title="Key architectural decisions and their rationale"]
--
