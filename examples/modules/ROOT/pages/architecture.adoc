...
--
The `expandOutgoingMacros()` and `expandIncomingMacros()` methods replace
`traceability:outgoing[]` and `traceability:incoming[]` placeholders with
formatted AsciiDoc content showing the item's relationships. Rendering is
opt-in via the `:traceability-links:` document attribute. Display style
(list, table, inline), sort order (target ID, title, relation type), and
collapsible sections are configurable via document attributes. Inline
relationship macros (`addresses:REQ-001[]`) are always stripped from
output — they are pure data markers; the link macros are the sole
rendering mechanism.
