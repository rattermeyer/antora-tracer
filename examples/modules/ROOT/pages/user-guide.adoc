...
[IMPORTANT]
====
Inline relationship macros (`addresses:REQ-001[]`) are *always invisible* in rendered output. They are pure data markers stored in the traceability graph. To display relationships in your documentation, use the `traceability:links[]` macro described below.

This keeps your source clean — type the relationship once, let the macro handle display.
====

=== The `traceability:links[]` Macro

Place `traceability:links[]` inside an item block to render all outgoing relationships as a formatted list:
...
| `:traceability-links:`
| `true` to enable the macro. Without it, `traceability:links[]` text is stripped.
