...
--
The system SHALL detect when a newly added relationship forms a complementary pair with an existing relationship — the existing edge goes from B to A with a type that is the configured inverse of the new edge's type — and merge them into a single bidirectional edge instead of storing two directed edges. Merging SHALL follow first-writer-wins: the first relationship stored remains as the canonical entry, and the second call to `addRelationship` is silently dropped.
...
--
The CLI SHALL provide a `query` subcommand that parses AsciiDoc source files, builds the in-memory traceability graph, and answers one structural question per invocation. No Antora build SHALL be required.
...
[#REQ-130, item, role=requirement, title="LinkResolver produces indexify-aware links"]
--
...
[#REQ-131, item, role=requirement, title="DOCX wrapper script converts assembled AsciiDoc to DOCX"]
--
...
[#REQ-132, item, role=requirement, title="DOCX assembler configs mirror PDF assembler configs"]
--
...
[#REQ-133, item, role=requirement, title="PDF playbook includes DOCX assembler configurations"]
--
...
[#REQ-134, item, role=requirement, title="pandoc is available in the build environment"]
--
