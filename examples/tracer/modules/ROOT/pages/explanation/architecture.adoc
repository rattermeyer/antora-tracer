...
traceability output directory. Each event handler is self-contained, does
...
via `contentCatalog.addFile()`. Registration happens before document
conversion, so `attachment$traceability/...` xrefs in nav files and pages
resolve without manual file copies. Matrices are registered under every
