...
--
`GraphDiff.diffGraphs(prev, next)` compares two graph snapshots by stable item ID, classifying each item as added, removed, or modified without hardcoding any role name.
...
graph diffing (`diff`), and item lifecycle management (`archive`,
`remove`).
