...
`.adoc` extension, producing `../../component/page.html#ID` links. With Antora's
default `indexify` URL style, pages at the module root produce
`pagename/index.html` instead of `pagename.html`. The `relativePathPrefix`
option allows configurable base paths, and the `indexify` option (default
`true`) controls whether root-level pages use `index.html`. When no
...
| Pass 2
| `expandRelationMacros(file, macroName)` — replace `traceability:links[]`, `traceability:incoming[]`, and `traceability:links[]` with formatted relationship lists (grouped by type, configurable style/order); macroName is `outgoing`, `incoming`, or `links`.
| Pages only
