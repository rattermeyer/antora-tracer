...
:keywords: quality, pdf, compatibility, macros, output

include::partial$quality/qa-pdf-compatibility.adoc[]

== Context

The `tracer:outgoing[]` and `tracer:incoming[]` macros expand to relationship lists.
If the macros generate raw HTML (for example `<div>` or `<span>` tags), the output works in the HTML site but breaks in the PDF build — asciidoctor-pdf cannot render raw HTML in AsciiDoc documents.
The output must use standard AsciiDoc constructs that work identically across backends.

== Trigger

An Antora build is executed with `:traceability-links: true` enabled, and `tracer:outgoing[]` or `tracer:incoming[]` macros are expanded.
