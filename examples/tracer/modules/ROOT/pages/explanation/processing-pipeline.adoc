...
Files from components named by the `excludeComponents` extension option are filtered out before this pass, so excluded components never enter the graph and receive no macro expansion or generated attachments.
...
. **Finalization**: The standalone output directory (matrices, coverage, overview) is written at `sitePublished` from the full graph, so it spans every component and version. The `graph.json` snapshot is not part of this pass — it is registered during `contentClassified` from the working graph, so each published version keeps its own copy.
