...
`traceability:graph[]` and `traceability:graph-coverage[]` placeholders are replaced with rendered diagram image references.
The graph's `toDot()` and `toVegaLite()` methods generate the diagram source.
