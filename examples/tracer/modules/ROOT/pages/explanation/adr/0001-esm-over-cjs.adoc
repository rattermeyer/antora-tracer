...
* Antora loads extensions via `require()` which expects CJS. Every ESM `.js` file that Antora's extension loader touches needs a `.cjs` wrapper — not just third-party packages. This is the primary integration friction: the project's own extension entry point must be wrapped.
