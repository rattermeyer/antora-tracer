...
Known limitations (for example `]` inside quoted attribute values) have proven fixable and the pattern is well-understood.
...
* Fragile — each edge case requires a regex fix (for example `]` in titles, indented macros, escaped colons)
