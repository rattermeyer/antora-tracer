...
*Sphinx-Needs* provides `needflow` for in-page graph visualization with PlantUML or Graphviz. *Antora Tracer* provides `traceability:graph[]` and `traceability:graph-coverage[]` macros for in-page graphviz/DOT visualization, plus Neo4j export for external graph queries.
