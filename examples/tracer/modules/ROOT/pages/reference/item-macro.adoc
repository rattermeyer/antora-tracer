...
Older versions used role-specific block macros (`[req]`, `[imp]`, `[test]`). These are detected with a deprecation error:

----
Error: Deprecated macro [req] found. Use [item, role=requirement] instead.
----

See xref:how-to/write-traceable-items.adoc[How to write traceable items] for migration guidance.
