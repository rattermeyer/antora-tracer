...
Two-pass approach: find `[item]` block macros, then scan content for inline relationship macros (`<type>:<TARGET-ID>[]`). Supports verbatim block exclusion, old macro detection, source file and line tracking.
...
Event handlers: `contentClassified` (process items), `sitePublished` (generate matrices and export).
