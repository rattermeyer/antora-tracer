...
Items are matched by ID: present only in `next` are `added`, only in `prev` are `removed`, and survivors are compared field-by-field (`title`, `content`, `role`, `status`, `attributes`) for `modified`.
