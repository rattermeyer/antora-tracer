...
The `leads_to` and `informs` relations are now valid from `use_case` items.
The `motivated_by` relation is valid from `requirement` back to `use_case`.
The extension will validate that only these relation types are used between these roles.
...
The `extends` field deep-merges — your roles are added, relations are merged, and matrices are appended.
