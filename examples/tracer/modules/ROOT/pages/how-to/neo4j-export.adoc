...
[source,bash]
----
# Export a local directory as CSV
npx antora-tracer export neo4j -i modules/ROOT/pages/ --format csv -o ./neo4j-export/

# Export the complete graph from an Antora playbook as Cypher
npx antora-tracer export neo4j antora-playbook.yml --format cypher -o ./neo4j-export/
----

The `-i` flag processes a local directory first (parsing items and relationships), then exports the graph.
Pass a playbook path instead to harvest every component and repository in that playbook and export the complete graph.
