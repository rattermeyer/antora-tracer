...
Pushing the `v0.7.0` tag triggers the `release` workflow, which stages the package with an npm provenance attestation and attaches a cosign-signed SBOM.
A maintainer then approves the staged version to publish it:
