...
== Supersede and retire items

When an item replaces another, the successor declares a `supersedes` relationship — no `status` attribute is needed.
Superseded items leave current matrices and render a `(superseded by ...)` marker.

Once a superseded item has no incoming functional links, it is *orphaned* and can be moved or removed:

[source,bash]
----
npx antora-tracer archive REQ-129   <1>
npx antora-tracer remove REQ-129    <2>
----
<1> Moves the block to the module's `superseded.adoc` page
<2> Deletes the block after confirmation (git preserves history)
