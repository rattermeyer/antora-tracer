The extension is composed of eleven components organized in four layers.
Arrows indicate dependencies, with constructor-injected dependencies labeled explicitly.
...
| `TraceabilityGraph`
| In-memory directed graph. Forward and reverse relationship indexes for O(1) lookups. Provides query methods (`findPath`, `getImpactAnalysis`), validation (dangling refs, circular refs), visualization (`toDot`, `toVegaLite`), and ID allocation (`getNextId`).
