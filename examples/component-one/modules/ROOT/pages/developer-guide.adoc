...
- *Synchronous init* — the Antora extension constructor initializes the traceability graph synchronously (config/preset loading is a synchronous file read) and registers event handlers only after the graph is ready, avoiding any race with the first `contentClassified` event
...
Invalid relations are stored anyway (graceful degradation) with a warning logged. The `validate()` method also runs `findCircularReferences()`, a DFS-based cycle detector that reports circular references with the full cycle path.
...
. Constructor initializes `RequirementsTraceabilityExtension` synchronously (config/preset loading is a synchronous file read)
. Event handlers are registered *after* initialization so the graph is always ready before any event fires
...
[ditaa]
----
register() → constructor ──loadConfig()             (from playbook)
                 │        └─initializeExtension()    (sync: preset or config)
                 ├─ create RequirementsTraceabilityExtension
...
Initialization is synchronous, so by the time the constructor returns, the
traceability graph is fully loaded and all event handlers are registered.
There is no race where a `contentClassified` event could fire before the
graph is ready.
...
| `next-id` | Next available ID for prefix | `-p <prefix>`, `-i <path>`
