  ...
  context.on('componentsRegistered', ({ contentCatalog }) => {
    ...
    for (const c of components) {
      for (const cv of c.versions) {
        ...
        process.stderr.write('[NAV-DEBUG] Component ' + cv.name + ' ' + cv.version + ' nav files: ' + navFiles.map(f => f.path).join(', ') + '\n')
        // Check assembler config
        const src = [...(cv.origins || [])].find(o => o.descriptor?.ext?.assembler)
        process.stderr.write('[NAV-DEBUG] Assembler config: ' + JSON.stringify(src?.descriptor?.ext?.assembler) + '\n')
      }
    }
  })
