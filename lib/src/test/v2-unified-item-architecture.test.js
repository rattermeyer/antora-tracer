...
`;
            ...
            expect(result.items[0].title).to.equal('Test Requirement');
        });
...
describe('DocumentParser', () => {
    describe('Item Parsing', () => {
        it('should parse item with all attributes', () => {
            ...
            const content = `
[item, id=REQ-001, role=requirement, title="Test", status=approved]
====
Content here
====
`;
            ...
            expect(result.items[0].id).to.equal('REQ-001');
            expect(result.items[0].role).to.equal('requirement');
            expect(result.items[0].title).to.equal('Test');
...
describe('TraceabilityGraph', () => {
    describe('Graph Operations', () => {
        ...
        it('should add and retrieve relationships', () => {
            const graph = new TraceabilityGraph();
            const item1 = { id: 'REQ-001', role: 'requirement', title: 'Req', attributes: {}, sourceFile: 'test.adoc', sourceLine: 1 };
            const item2 = { id: 'DES-001', role: 'design', title: 'Design', attributes: {}, sourceFile: 'test.adoc', sourceLine: 5 };
            graph.addItem(item1);
            graph.addItem(item2);
            const relationship = {
                id: 'REL-001',
        ...
        it('should get role statistics', () => {
            const graph = new TraceabilityGraph();
            const item1 = { id: 'REQ-001', role: 'requirement', title: 'Req 1', attributes: {}, sourceFile: 'test.adoc', sourceLine: 1 };
            const item2 = { id: 'REQ-002', role: 'requirement', title: 'Req 2', attributes: {}, sourceFile: 'test.adoc', sourceLine: 5 };
            const item3 = { id: 'DES-001', role: 'design', title: 'Design', attributes: {}, sourceFile: 'test.adoc', sourceLine: 10 };
            graph.addItem(item1);
            graph.addItem(item2);
            graph.addItem(item3);
