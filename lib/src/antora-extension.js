...
export class AntoraTraceabilityExtension {
    ...
    generateLinksAsciiDoc(grouped, style, _sourceFile) {
        if (grouped.length === 0)
            return '';
        if (style === 'table')
            return this.generateTableStyle(grouped);
        if (style === 'inline')
            return this.generateInlineStyle(grouped);
        return this.generateListStyle(grouped);
    }
    generateListStyle(grouped) {
        const lines = [];
    ...
    substituteRelationshipLinks(content, currentFile) {
        ...
        return content.replace(relRegex, (_match, _relType, targetId, matchPos) => {
