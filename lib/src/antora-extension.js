...
export class AntoraTraceabilityExtension {
    ...
    findItemBlocks(content) {
        ...
        while ((m = macroStartRe.exec(content)) !== null) {
            if (this.isInsideRange(m.index, verbatimRanges))
                continue;
    ...
    expandConfigGraphMacros(file) {
        ...
        try {
            ...
            while ((match = macroRegex.exec(content)) !== null) {
                if (this.isInsideRange(match.index, protectedRanges))
                    continue;
    ...
    substituteRelationshipLinks(content) {
        ...
        for (const range of ranges) {
            if (range.start < pos)
                continue; // skip nested ranges already protected
