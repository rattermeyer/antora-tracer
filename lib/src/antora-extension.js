 ...
 * }
 */
import { existsSync, mkdirSync, writeFileSync, readdirSync, rmSync, statSync, copyFileSync, } from "node:fs";
...
export class AntoraTraceabilityExtension {
    ...
    loadConfig() {
        try {
            ...
            if (!extensions)
                return {};
            const extEntry = extensions.find((e) => e.require === "antora-tracer/antora-extension" ||
    ...
    expandOutgoingMacros(file) {
        ...
        try {
            ...
            for (const { itemId, bodyStart } of blocks) {
                const bodyContent = content.slice(bodyStart, content.indexOf("\n--\n", bodyStart));
                ...
                while ((macroMatch = macroRegex.exec(bodyContent)) !== null) {
                    ...
                    const generated = this.generateLinksAsciiDoc(groupEntries, style, currentFile, collapsible, currentComponent, currentModule);
                    replacements.push({
                        start: macroStart,
                        end: macroEnd,
                        text: generated,
            ...
            for (const { itemId, bodyStart } of blocks) {
                ...
                while ((macroMatch = macroRegex.exec(bodyContent)) !== null) {
                    ...
                    for (const rel of rels) {
                        ...
                        const configInvLabels = this.traceability.configLoader?.getConfig()?.inverseLabels;
                        const inverseType = configInvLabels?.[rel.type] ??
                            INVERSE_MAP[rel.type] ??
                            rel.type;
    ...
    expandLinksMacros(file) {
        ...
        try {
            ...
            const docAttrs = this.parseDocAttributes(content);
            const linksEnabled = this.isLinksEnabled(docAttrs);
            if (!content.includes("traceability:links[]"))
                return;
            const style = this.getLinksStyle(docAttrs);
            const order = this.getLinksOrder(docAttrs);
            const collapsible = this.getCollapsible(docAttrs);
                ...
                while ((macroMatch = macroRegex.exec(bodyContent)) !== null) {
                    ...
                    if (outgoingRels.length > 0) {
                        ...
                        if (grouped.size > 0) {
                            ...
                            outgoingParts.push(this.generateLinksAsciiDoc(groupEntries, style, currentFile, collapsible, currentComponent, currentModule));
                        }
                    }
                    ...
                    if (incomingRels.length > 0) {
                        ...
                        if (grouped.size > 0) {
                            ...
                            incomingParts.push(this.generateLinksAsciiDoc(groupEntries, style, currentFile, collapsible, currentComponent, currentModule));
                        }
                    }
    ...
    expandGraphMacros(file) {
        ...
        try {
            ...
            const content = contentsBuffer.toString("utf8");
            const docAttrs = this.parseDocAttributes(content);
            if (!content.includes("traceability:graph["))
                return;
    ...
    expandCoverageMacros(file) {
        ...
        try {
            ...
            const content = contentsBuffer.toString("utf8");
            const docAttrs = this.parseDocAttributes(content);
            if (!content.includes("traceability:graph-coverage["))
                return;
            const graphEnabled = this.isGraphEnabled(docAttrs);
            const blocks = this.findItemBlocks(content);
            const replacements = [];
     ...
     */
    syncMatricesToAttachments(event, traceabilityDir, outputDir) {
        try {
            const contentCatalog = event.contentCatalog;
            if (!contentCatalog) {
            ...
            const versionSegments = new Map(); // component -> versionSegment
            for (const file of attachmentFiles) {
                if (file.src?.component && file.src?.version && !versionSegments.has(file.src.component)) {
                    ...
                    const match = outPath.match(/^([^/]+)\/([^/]+)\/_attachments\//);
                    if (match) {
                ...
                if (components) {
                    for (const comp of components) {
                        for (const cv of comp.versions || []) {
                            if (cv.version)
                                versionSegments.set(comp.name, cv.version);
                        }
                    }
                }
            }
            ...
            for (const [componentName, versionSegment] of versionSegments) {
                const attachDir = join(outputDir, componentName, versionSegment, "_attachments", "traceability");
                ...
                this.logger.info(`Synced matrices to ${componentName}/${versionSegment}/_attachments/traceability/`);
            }
        }
        catch (error) {
            this.logger.warn(`Failed to sync matrices to attachments: ${error.message}`);
        }
    }
