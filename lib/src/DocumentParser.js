 ...
 */
import { RENDERING_MACRO_LOOKAHEAD, } from "./types.js";
/**
...
export class DocumentParser {
     ...
     */
    parse(content, sourceFile, component, module, pubUrl, version) {
        // Validate input
    ...
    parseInlineMacrosFromItems(_content, sourceFile, result) {
        ...
        for (const item of result.items) {
            ...
            const inlineMacroRegex = new RegExp(`(?<!\\\\)${RENDERING_MACRO_LOOKAHEAD}([a-zA-Z][a-zA-Z0-9_-]*:[A-Z0-9_-]+(?:\\s*,\\s*[A-Z0-9_-]+)*)\\[`, "g");
            let match;
            while ((match = inlineMacroRegex.exec(itemContent)) !== null) {
                // Skip macros inside backtick code spans
                if (this.isInsideRange(match.index, backtickRanges))
                    continue;
                const macro = match[1];
    ...
    extractBlock(content, startIndex) {
        ...
        if (!closeMatch)
            return null;
        return content.substring(startIndex, closeMatch.index + closeMatch[0].length);
    }
    extractBody(block) {
