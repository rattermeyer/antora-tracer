...
config({ quiet: true });
// Import extension
import { BUILT_IN_PRESETS, ConfigLoader, deserializeSnapshot, diffGraphs, diffSnapshots, harvestSiteFiles, RequirementsTraceabilityExtension, serializeSnapshot, } from "./index.js";
...
async function createExtension(options) {
    ...
    try {
        ...
        if (mergedOptions.preset) {
            return RequirementsTraceabilityExtension.createWithPreset(mergedOptions.preset);
        }
 ...
 */
async function fetchNextId(endpoint, token, prefix, timeoutMs = 10_000) {
 ...
 */
function collectAdocFiles(inputPath) {
    ...
    if (stat.isDirectory()) {
        const files = [];
        const entries = readdirSync(resolvedPath, { recursive: true });
        for (const entry of entries) {
            const fullPath = resolve(resolvedPath, entry);
            if (statSync(fullPath).isFile() && entry.endsWith(".adoc")) {
                files.push({
                    path: entry,
...
/**
 * Resolve an item's sourceFile (relative to the input directory) to an
 * absolute filesystem path.
 */
function resolveSourceFilePath(input, sourceFile) {
    if (!sourceFile) {
...
/**
 * Extract an item's full block (header line through the closing delimiter)
 * given the file content and the item's 1-indexed source line.
 */
function extractItemBlock(content, sourceLine) {
    ...
    if (headerIdx < 0 || headerIdx >= lines.length)
        return null;
    if (!/\[#[^,\]]+,\s*item,?/.test(lines[headerIdx]))
        return null;
    let openIdx = -1;
    ...
    if (openIdx === -1)
        return null;
    const delimiter = lines[openIdx].trim();
    let closeIdx = -1;
    ...
    if (closeIdx === -1)
        return null;
    return {
        block: lines.slice(headerIdx, closeIdx + 1).join("\n"),
...
presetProgram
    ...
    console.log(chalk.cyan("Usage: antora-tracer process --preset <name> -i <file>"));
    ...
    try {
        ...
        for (const [sourceRole, targets] of Object.entries(relations)) {
            ...
            for (const [targetRole, typeMap] of Object.entries(targets)) {
                console.log(`    -> ${targetRole}: [${Object.keys(typeMap).join(", ")}]`);
            }
        }
        console.log("");
        if (preset.traceability.matrices &&
...
`;
            ...
            console.log(chalk.cyan(`  1. Process: antora-tracer process -i ${samplePath} --config traceability.yml`));
            console.log(chalk.cyan(`  2. Generate matrix: antora-tracer matrix --config traceability.yml -o matrix.html -f html`));
            console.log(chalk.cyan(`  3. Validate: antora-tracer validate -i ${samplePath} --config traceability.yml`));
        }
    }
    catch (_error) {
        console.error(chalk.red(`Error: Preset '${name}' not found`));
...
configProgram
    ...
    try {
        ...
        if (!configPath) {
            ...
            console.log(chalk.yellow("Try: antora-tracer config validate --config traceability.yml"));
            console.log(chalk.yellow("Or: antora-tracer config validate -c /path/to/config.yml"));
            process.exit(1);
        }
    ...
    try {
        ...
        if (isDryRun(options)) {
            ...
            if (result.relationshipsFile)
                console.log(chalk.green(`  Relationships file: ${result.relationshipsFile}`));
...
program
    ...
    try {
        ...
        if (idAllocation?.endpoint && !options.local) {
            const nextId = await fetchNextId(idAllocation.endpoint, idAllocation.token, options.prefix);
            console.log(nextId);
            return;
        }
        if (!options.input) {
...
program
    .command("seed")
    .description("Export allocator seed values (next ID per prefix) for the ID allocation server")
    .argument("[playbook]", "Antora playbook to harvest (all components/repos)")
 ...
 * `query` command.
 */
async function buildQueryGraph(cmd) {
...
queryProgram
    ...
    if (json) {
        ...
        console.log(formatTable(["ID", "Role", "Relation", "File", "Line"], rows));
    }
});
queryProgram
    ...
    .action(async (id, role, _options, cmd) => {
    const { json } = cmd.parent.opts();
    ...
    if (snapshot) {
        try {
            const data = deserializeSnapshot(readFileSync(resolve(process.cwd(), snapshot), "utf8"));
            const extension = await createExtension({});
...
queryProgram
    ...
    const orphans = graph
        .getAllItems()
        .filter((item) => graph.getRelationships(item.id).length === 0 &&
...
queryProgram
    ...
    if (json) {
        ...
        console.log(formatTable(["ID", "Role", "Title", "Superseded by", "File"], rows));
    }
});
queryProgram
...
supersessionProgram
    .command("check <id>")
    .description("Report successors and functional links requiring review for an item")
    ...
    if (json) {
        ...
        if (options.impact) {
            ...
            console.log(`Transitive impact: ${impact.length > 0 ? impact.join(", ") : "(none)"}`);
        }
    }
});
program
    ...
    console.log(formatTable(["Kind", "ID", "Role", "Changed fields"], itemRows));
...
program
    .command("site-graph")
    .description("Build the full cross-source traceability graph from an Antora playbook and emit a JSON snapshot")
    .argument("<playbook>", "Path to the Antora playbook file")
    .option("-o, --out <path>", "Write the snapshot to a file (defaults to stdout)")
    .action(async (playbook, options) => {
    try {
        const extension = await createExtension({});
        const files = await harvestSiteFiles(playbook);
...
program
    ...
    .action((options) => {
    try {
        const prev = deserializeSnapshot(readFileSync(resolve(process.cwd(), options.from), "utf8"));
        const next = deserializeSnapshot(readFileSync(resolve(process.cwd(), options.to), "utf8"));
...
program
    ...
    if (!graph.isSuperseded(id)) {
        console.error(chalk.red(`Item ${id} is not superseded — only superseded items can be archived.`));
        process.exit(1);
    }
    const sourcePath = resolveSourceFilePath(options.input, item.sourceFile);
    const content = readFileSync(sourcePath, "utf8");
    const extracted = extractItemBlock(content, item.sourceLine);
    if (!extracted) {
        console.error(chalk.red(`Could not locate the block for ${id} in ${item.sourceFile}:${item.sourceLine ?? "?"}. No change made.`));
        process.exit(1);
    }
    const lines = content.split("\n");
    lines.splice(extracted.startLine - 1, extracted.endLine - extracted.startLine + 1);
    writeFileSync(sourcePath, lines.join("\n"), "utf8");
    const supersededPath = join(dirname(sourcePath), "superseded.adoc");
    if (!existsSync(supersededPath)) {
        writeFileSync(supersededPath, "= Superseded Items\n\n", "utf8");
    }
    const existing = readFileSync(supersededPath, "utf8");
    writeFileSync(supersededPath, `${existing}${existing.endsWith("\n") ? "\n" : ""}${extracted.block}\n`, "utf8");
    console.log(chalk.green(`Archived ${id} to ${supersededPath}`));
});
program
    .command("remove <id>")
    .description("Remove an orphaned or isolated item block")
    .option("-i, --input <path>", "Input file or directory", ".")
    .action(async (id, options) => {
    const extension = await createExtension({});
    extension.processFiles(collectAdocFiles(options.input));
    ...
    const orphaned = graph.isOrphaned(id);
    const isolated = graph.getRelationships(id).length === 0 &&
        graph.getReverseRelationships(id).length === 0;
    if (!orphaned && !isolated) {
        console.error(chalk.red(`Item ${id} is neither orphaned nor isolated — nothing to remove.`));
        process.exit(1);
    }
    const sourcePath = resolveSourceFilePath(options.input, item.sourceFile);
    const content = readFileSync(sourcePath, "utf8");
    const extracted = extractItemBlock(content, item.sourceLine);
    if (!extracted) {
        console.error(chalk.red(`Could not locate the block for ${id}. No change made.`));
        process.exit(1);
    }
    console.log(chalk.blue(`Will remove ${id} from ${item.sourceFile}:`));
    ...
    if (orphaned) {
        ...
        const answer = await promptLine(`This item is NOT superseded. Type the ID to confirm removal: `);
        if (answer !== id) {
            console.log("Confirmation did not match. No change made.");
            return;
        }
    }
    const lines = content.split("\n");
    lines.splice(extracted.startLine - 1, extracted.endLine - extracted.startLine + 1);
    writeFileSync(sourcePath, lines.join("\n"), "utf8");
    console.log(chalk.green(`Removed ${id} from ${item.sourceFile}`));
});
