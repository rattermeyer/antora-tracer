...
`;
            ...
            expect(result.requirements[0].id).to.equal('REQ-001');
            expect(result.requirements[0].title).to.equal('User Authentication');
            expect(result.requirements[0].content).to.equal('The system shall require user authentication.');
            expect(result.requirements[0].status).to.equal('draft');
        });
...
`;
            const result = parser.parse(content, 'test.adoc');
            expect(result.requirements).to.have.lengthOf(1);
            expect(result.requirements[0].status).to.equal('approved');
        });
...
`;
            const result = parser.parse(content, 'test.adoc');
            expect(result.requirements).to.have.lengthOf(1);
            expect(result.requirements[0].id).to.match(/^REQ-/);
            expect(result.requirements[0].content).to.equal('Auto-generated requirement');
            expect(result.requirements[0].title).to.match(/^Requirement REQ-/);
        });
...
====
`;
            expect(() => parser.parse(content, 'test.adoc')).to.throw('Duplicate requirement ID: REQ-DUP');
        });
    });
...
`;
            const result = parser.parse(content, 'test.adoc');
            expect(result.implementations).to.have.lengthOf(1);
            expect(result.implementations[0].id).to.equal('IMP-001');
            expect(result.implementations[0].title).to.equal('User Auth Implementation');
            expect(result.implementations[0].content).to.equal('Implementation of user authentication.');
        });
...
`;
            const result = parser.parse(content, 'test.adoc');
            expect(result.implementations).to.have.lengthOf(1);
            expect(result.implementations[0].id).to.match(/^IMP-/);
            expect(result.implementations[0].content).to.equal('Auto-generated implementation');
            expect(result.implementations[0].title).to.match(/^Implementation IMP-/);
        });
...
====
`;
            expect(() => parser.parse(content, 'test.adoc')).to.throw('Duplicate implementation ID: IMP-DUP');
        });
    });
...
`;
            const result = parser.parse(content, 'test.adoc');
            expect(result.tests).to.have.lengthOf(1);
            expect(result.tests[0].id).to.equal('TEST-001');
            expect(result.tests[0].title).to.equal('Authentication Test');
            expect(result.tests[0].content).to.equal('Test that user authentication works.');
        });
...
`;
            const result = parser.parse(content, 'test.adoc');
            expect(result.tests).to.have.lengthOf(1);
            expect(result.tests[0].id).to.match(/^TEST-/);
            expect(result.tests[0].content).to.equal('Auto-generated test');
            expect(result.tests[0].title).to.match(/^Test TEST-/);
        });
...
====
`;
            expect(() => parser.parse(content, 'test.adoc')).to.throw('Duplicate test ID: TEST-DUP');
        });
    });
...
`;
            ...
            expect(result.documents[0].id).to.equal('DOC-001');
            expect(result.documents[0].title).to.equal('Architecture Document');
            expect(result.documents[0].content).to.equal('System architecture documentation.');
        });
...
`;
            const result = parser.parse(content, 'test.adoc');
            expect(result.documents).to.have.lengthOf(1);
            expect(result.documents[0].id).to.match(/^DOC-/);
            expect(result.documents[0].content).to.equal('Auto-generated document');
            expect(result.documents[0].title).to.match(/^Document DOC-/);
        });
    });
...
`;
            const result = parser.parse(content, 'test.adoc');
            expect(result.requirements).to.have.lengthOf(2);
            expect(result.relationships).to.have.lengthOf(1);
            expect(result.relationships[0].fromId).to.equal('REQ-001');
            expect(result.relationships[0].targetId).to.equal('REQ-002');
            expect(result.relationships[0].type).to.equal('satisfies');
        });
...
`;
            ...
            expect(result.requirements).to.have.lengthOf(1);
            expect(result.relationships).to.have.lengthOf(1);
            expect(result.relationships[0].fromId).to.equal('IMP-001');
            expect(result.relationships[0].targetId).to.equal('REQ-001');
            expect(result.relationships[0].type).to.equal('implements');
        });
...
`;
            ...
            expect(result.requirements).to.have.lengthOf(1);
            expect(result.relationships).to.have.lengthOf(1);
            expect(result.relationships[0].fromId).to.equal('TEST-001');
            expect(result.relationships[0].targetId).to.equal('REQ-001');
            expect(result.relationships[0].type).to.equal('tests');
        });
...
`;
            const result = parser.parse(content, 'test.adoc');
            expect(result.relationships).to.have.lengthOf(1);
            expect(result.relationships[0].type).to.equal('verifies');
        });
...
`;
            ...
            expect(result.requirements).to.have.lengthOf(1);
            expect(result.relationships).to.have.lengthOf(1);
            expect(result.relationships[0].fromId).to.equal('DOC-001');
            expect(result.relationships[0].targetId).to.equal('REQ-001');
            expect(result.relationships[0].type).to.equal('documents');
        });
...
`;
            const result = parser.parse(content, 'test.adoc');
            expect(result.relationships).to.have.lengthOf(1);
            expect(result.relationships[0].type).to.equal('depends');
        });
...
`;
            const result = parser.parse(content, 'test.adoc');
            expect(result.relationships).to.have.lengthOf(1);
            expect(result.relationships[0].type).to.equal('requires');
        });
...
`;
            const result = parser.parse(content, 'test.adoc');
            expect(result.requirements).to.have.lengthOf(3);
...
`;
            ...
            expect(result.relationships.length).to.be.at.least(4);
        });
...
`;
            ...
            expect(result.requirements[0].sourceFile).to.equal('my-document.adoc');
        });
...
`;
            ...
            expect(result.requirements[0].sourceLine).to.be.a('number');
            expect(result.requirements[0].sourceLine).to.be.greaterThan(0);
        });
    });
});
