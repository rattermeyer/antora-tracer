...
`;
            ...
            (0, chai_1.expect)(req).to.exist;
            (0, chai_1.expect)(req.id).to.equal('REQ-001');
            (0, chai_1.expect)(req.title).to.equal('Test Requirement');
            (0, chai_1.expect)(req.content).to.contain('This is a test requirement');
        });
...
`;
            ...
            (0, chai_1.expect)(req.id).to.match(/REQ-\d+-\d+/);
            (0, chai_1.expect)(req.title).to.equal('Auto ID Test');
        });
...
`;
            ...
            (0, chai_1.expect)(req.sourceFile).to.equal('test.adoc');
            (0, chai_1.expect)(req.sourceLine).to.be.a('number');
        });
    });
    describe('Requirement Validation', function () {
        it('should detect duplicate requirement IDs', async function () {
            ...
            let thrown = null;
            try {
                await extension.process(content2);
            }
            catch (e) {
                thrown = e;
            }
            (0, chai_1.expect)(thrown).to.not.be.null;
            (0, chai_1.expect)(thrown.message).to.include('Duplicate requirement ID: REQ-003');
        });
        it('should warn about non-standard ID formats', function () {
            ...
            (0, chai_1.expect)(warningCaptured).to.contain('Non-standard requirement ID format');
        });
    });
...
`;
        ...
        it('should add relationships between requirements', function () {
            ...
            (0, chai_1.expect)(rels).to.have.lengthOf(1);
            (0, chai_1.expect)(rels[0].type).to.equal('satisfies');
            (0, chai_1.expect)(rels[0].targetId).to.equal('REQ-101');
        });
        it('should store relationship with correct properties', function () {
            extension.addRelationship('REQ-100', 'REQ-101', 'satisfies');
            const rels = extension.graph.getRelationships('REQ-100');
            (0, chai_1.expect)(rels[0].fromId).to.equal('REQ-100');
            (0, chai_1.expect)(rels[0].targetId).to.equal('REQ-101');
            (0, chai_1.expect)(rels[0].type).to.equal('satisfies');
        });
    });
...
`;
        ...
        it('should generate traceability matrices', function () {
            ...
            (0, chai_1.expect)(matrix.generatedAt).to.be.a('string');
        });
    });
...
`;
            ...
            (0, chai_1.expect)(result).to.contain('Test Document');
            (0, chai_1.expect)(result).to.contain('REQ-300');
        });
...
`;
            const result = extension.process(content);
            (0, chai_1.expect)(extension.graph.getAllRequirements()).to.have.lengthOf(3);
        });
    });
});
