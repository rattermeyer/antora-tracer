...
describe('Performance Optimization', function () {
    ...
    describe('Large Dataset Handling', function () {
        ...
        it('should handle 1000 requirements efficiently', function () {
            ...
            (0, chai_1.expect)(addTime).to.be.lessThan(1000); // Should add 1000 nodes in under 1 second
            // Verify all were added
            const allReqs = extension.graph.getAllRequirements();
            (0, chai_1.expect)(allReqs).to.have.lengthOf(1000);
            // Test retrieval
            ...
            (0, chai_1.expect)(retrieveTime).to.be.lessThan(100); // Should retrieve 1000 nodes in under 100ms
        });
        it('should handle complex relationship graph efficiently', function () {
            ...
            const addTime = Date.now() - startTime;
            (0, chai_1.expect)(addTime).to.be.lessThan(2000); // Should add 1000 nodes and 500 relationships in under 2 seconds
            ...
            (0, chai_1.expect)(allRelationships).to.have.lengthOf(500);
        });
        it('should handle path finding in large graphs efficiently', function () {
            ...
            (0, chai_1.expect)(findTime).to.be.lessThan(100); // Should find path in under 100ms
            (0, chai_1.expect)(path).to.not.be.null;
            (0, chai_1.expect)(path.length).to.equal(100);
        });
        it('should handle impact analysis in large graphs efficiently', function () {
            ...
            (0, chai_1.expect)(analysisTime).to.be.lessThan(100); // Should complete in under 100ms
            (0, chai_1.expect)(impacted).to.have.lengthOf(100);
            (0, chai_1.expect)(impacted).to.include('REQ-0001');
            (0, chai_1.expect)(impacted).to.include('REQ-0100');
        });
        it('should handle coverage calculation in large graphs efficiently', function () {
            ...
            (0, chai_1.expect)(coverageTime).to.be.lessThan(100); // Should calculate in under 100ms
            (0, chai_1.expect)(coverage.totalRequirements).to.equal(500);
            (0, chai_1.expect)(coverage.requirementsWithImplementation).to.equal(250);
            (0, chai_1.expect)(coverage.requirementsWithTests).to.equal(125);
        });
    });
    describe('Caching Performance', function () {
        it('should cache getAllRequirements results', function () {
            ...
            for (let i = 0; i < 100; i++) {
                ...
                (0, chai_1.expect)(all).to.have.lengthOf(1000);
            }
            const cacheTime = Date.now() - startTime;
            (0, chai_1.expect)(cacheTime).to.be.lessThan(50); // Should be very fast with caching
        });
        it('should invalidate cache when nodes are added', function () {
            ...
            (0, chai_1.expect)(all).to.have.lengthOf(100);
            // Add one more
            extension.graph.addRequirement({
                id: 'REQ-101',
            ...
            (0, chai_1.expect)(all).to.have.lengthOf(101);
        });
    });
    describe('Memory Efficiency', function () {
        it('should not leak memory when clearing graph', function () {
            ...
            extension.clear();
            // Verify it's empty
            (0, chai_1.expect)(extension.graph.getAllRequirements()).to.have.lengthOf(0);
            (0, chai_1.expect)(extension.graph.size()).to.equal(0);
