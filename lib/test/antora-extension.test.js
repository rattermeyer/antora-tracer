...
function createMockContext(overrides = {}) {
    ...
    if (overrides.logs)
        ctx.logs = overrides.logs;
    if (overrides.events)
        ctx.events = overrides.events;
    if (overrides.fireEvent)
        ctx.fireEvent = overrides.fireEvent;
    return ctx;
}
/**
 ...
 */
function createContentClassifiedEvent(files) {
    return {
        contentCatalog: {
            findBy: ({ family: _family }) => files.map((f) => ({
                src: {
                    path: f.path,
                    module: f.module,
...
describe("AntoraTraceabilityExtension", () => {
    ...
    describe("Initialization", () => {
        it("should initialize with default configuration", async () => {
            ...
            const ext = new AntoraTraceabilityExtension(ctx);
            await waitForInit();
        ...
        it("should handle disabled extension without initializing", () => {
            ...
            expect(() => ext.getTraceabilityExtension()).to.throw("Traceability extension not initialized");
            expect(ctx.logs.some((l) => l.message.includes("is disabled"))).to.be
                .true;
        });
        it("should register event handlers on initialization", async () => {
            const ctx = createMockContext();
            new AntoraTraceabilityExtension(ctx);
            await waitForInit();
    ...
    describe("Content Processing", () => {
        it("should process AsciiDoc files when contentClassified fires", async () => {
            ...
            const ext = new AntoraTraceabilityExtension(ctx);
            await waitForInit();
            ctx.fireEvent("contentClassified", createContentClassifiedEvent([
...
====
`;
            ctx.fireEvent("contentClassified", createContentClassifiedEvent([{ path: "multi.adoc", content }]));
            const traceExt = ext.getTraceabilityExtension();
...
`),
            ...
            expect(items).to.have.lengthOf(2);
            const partialItem = items.find((i) => i.id === "REQ-100");
...
`),
    ...
    describe("Matrix Generation", () => {
        it("should generate matrix files on sitePublished when items exist", async () => {
            ...
            const _ext = new AntoraTraceabilityExtension(ctx);
            await waitForInit();
            // Populate with items
            ctx.fireEvent("contentClassified", createContentClassifiedEvent([
            ...
            const matrixFiles = files.filter((f) => f.startsWith("matrix-"));
        ...
        it("should generate matrices in configured formats", async () => {
            ...
            const htmlMatrices = files.filter((f) => f.startsWith("matrix-") && f.endsWith(".html"));
            const csvMatrices = files.filter((f) => f.startsWith("matrix-") && f.endsWith(".csv"));
    ...
    describe("Error Handling", () => {
        it("should log error and continue on processing failure", async () => {
            ...
            expect(ctx.logs.some((l) => l.level === "warn" && l.message.includes("broken.adoc"))).to.be.true;
        });
...
--
`;
            ctx.fireEvent("contentClassified", createContentClassifiedEvent([{ path: "test.adoc", content }]));
...
`;
            ...
            const macroWarns = ctx.logs.filter((l) => l.level === "warn" &&
...
`;
         ...
         */
        async function renderDoc(content) {
            const ctx = createMockContext({
                playbook: { output: { dir: tempDir }, extensions: [] },
            });
            const ext = new AntoraTraceabilityExtension(ctx);
...
`;
            ...
            const attachDir = join(testOutputDir, "test-component", "latest", "_attachments", "traceability");
            expect(existsSync(attachDir)).to.be.true;
            // Verify at least the CSV file was synced
            const files = readdirSync(attachDir);
            expect(files.some((f) => f.startsWith("matrix-"))).to.be.true;
        });
...
`;
            ...
            const attachDir = join(testOutputDir, "fallback-comp", "2.0.0", "_attachments", "traceability");
            expect(existsSync(attachDir)).to.be.true;
        });
    });
    // ========================================================================
...
`;
            ...
            const buildXref = ext.buildXref.bind(ext);
            // Cross-module: current is requirements, target is ROOT → should include "ROOT:"
            const xref = buildXref({ id: "ARC-001", title: "Auth Module", sourceFile: "architecture", component: "tracer", module: "ROOT" }, "index", "Auth Module", "tracer", "requirements");
...
`,
            ...
            const buildXref = ext.buildXref.bind(ext);
            const xref = buildXref({ id: "ARC-002", title: "Session", sourceFile: "architecture", component: "tracer", module: "ROOT" }, "architecture", "Session", "tracer", "ROOT");
        ...
        it("should handle item with no component/module (CLI context)", async () => {
            ...
            const xref = buildXref({ id: "REQ-001", title: "Test", sourceFile: "test" }, "other-page", "Test", undefined, undefined);
        ...
        it("should handle cross-component xref", async () => {
            ...
            const xref = buildXref({ id: "ITEM-001", title: "Other Item", sourceFile: "page", component: "other-comp", module: "ROOT" }, "my-page", "Other Item", "tracer", "ROOT");
