...
`;
        ...
        const targetRels = result.relationships.filter((r) => r.targetId === "TARGET");
...
`;
        ...
        const realRels = result.relationships.filter((r) => r.targetId === "DES-001");
        expect(realRels).to.have.lengthOf(1);
        // Backtick-enclosed macro should NOT be parsed
        const fakeRels = result.relationships.filter((r) => r.targetId === "TARGET");
    ...
    describe("Module and Component Context", () => {
        it("should set module and component on parsed items when provided", () => {
            ...
            const result = parser.parse(`[#REQ-001, item, role=requirement, title="Test"]
--
Content
--
`, "architecture", "tracer", "ROOT");
            expect(result.items).to.have.lengthOf(1);
            expect(result.items[0].component).to.equal("tracer");
            expect(result.items[0].module).to.equal("ROOT");
        });
        it("should leave module and component undefined when not provided", () => {
            const parser = new DocumentParser();
            const result = parser.parse(`[#REQ-001, item, role=requirement, title="Test"]
--
Content
...
`, "architecture");
            expect(result.items).to.have.lengthOf(1);
            expect(result.items[0].component).to.be.undefined;
            expect(result.items[0].module).to.be.undefined;
        });
    });
    describe("DocumentParser — Old Macro Detection", () => {
        it("should flag old [req] macro with suggestions in strict mode", () => {
            const parser = new DocumentParser();
            const content = `= Doc\n\n[req, id=REQ-001]\n====\nLegacy requirement\n====\n`;
            const result = parser.parse(content, "test.adoc");
            ...
            expect(error.message).to.include("role=requirement");
        });
        it("should flag old [imp] macro with suggested role", () => {
            const parser = new DocumentParser();
            const content = `= Doc\n\n[imp, id=IMP-001]\n====\nLegacy implementation\n====\n`;
            const result = parser.parse(content, "test.adoc");
            expect(result.errors).to.have.length.greaterThan(0);
            expect(result.errors[0].message).to.include("role=implementation");
        });
        it("should flag old macros with trailing attributes (comma form)", () => {
            const parser = new DocumentParser();
            const content = `= Doc\n\n[design, id=D-001, width=80]\n====\nLegacy design\n====\n\n[test, id=T-001]\n====\nLegacy test\n====\n`;
            const result = parser.parse(content, "test.adoc");
            expect(result.errors.length).to.be.greaterThanOrEqual(2);
            expect(result.errors.some((e) => e.message.includes("role=design"))).to
                .be.true;
            expect(result.errors.some((e) => e.message.includes("role=test"))).to
                .be.true;
        });
        it("should flag old macro with a whitespace separator", () => {
            const parser = new DocumentParser();
            const content = `= Doc\n\n[req id=REQ-001]\n====\nLegacy requirement\n====\n`;
            const result = parser.parse(content, "test.adoc");
            expect(result.errors).to.have.length.greaterThan(0);
            expect(result.errors[0].message).to.include("role=requirement");
        });
        it("should flag a bare old macro with no attributes", () => {
            const parser = new DocumentParser();
            const content = `= Doc\n\n[req]\n====\nLegacy bare requirement\n====\n`;
            const result = parser.parse(content, "test.adoc");
            expect(result.errors).to.have.length.greaterThan(0);
            expect(result.errors[0].message).to.include("role=requirement");
        });
        it("should not flag old macros inside verbatim blocks", () => {
            const parser = new DocumentParser();
            const content = `= Doc\n\n[source,asciidoc]\n----\n[req, id=REQ-X]\n====\nExample only\n====\n----\n`;
            const result = parser.parse(content, "test.adoc");
        ...
        it("should not misdetect prose words containing old-macro prefixes", () => {
            const parser = new DocumentParser();
            const content = `= Doc\n\nHere we discuss the red request and testimony of record.`;
            const result = parser.parse(content, "test.adoc");
            expect(result.errors).to.have.lengthOf(0);
            expect(result.warnings).to.have.lengthOf(0);
        });
        it("should emit warnings instead of errors in non-strict mode", () => {
            const parser = new DocumentParser({ strictMode: false });
            const content = `= Doc\n\n[req, id=REQ-001]\n====\nLegacy requirement\n====\n`;
            const result = parser.parse(content, "test.adoc");
            expect(result.errors).to.have.lengthOf(0);
            expect(result.warnings).to.have.length.greaterThan(0);
            expect(result.warnings[0].type).to.equal("old_macro");
        });
    });
});
