...
export class AntoraTraceabilityExtension {
    ...
    expandRelationMacros(file, macroName) {
        ...
        try {
            ...
            if (!content.includes(`traceability:${macroName}[]`))
                return;
            ...
            for (const { itemId, bodyStart } of blocks) {
                ...
                while ((macroMatch = macroRegex.exec(bodyContent)) !== null) {
                    ...
                    const text = linksEnabled
                        ? this.buildRelationMacroOutput(itemId, macroName, style, order, currentFile, collapsible, currentComponent, currentModule)
                        : "";
     ...
     */
    buildRelationMacroOutput(itemId, macroName, style, order, currentFile, collapsible, currentComponent, currentModule) {
        const directions = macroName === "links"
            ? ["outgoing", "incoming"]
            : [macroName];
        const parts = [];
