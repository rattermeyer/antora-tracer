...
export class AntoraTraceabilityExtension {
    ...
    substituteRelationshipLinks(content, currentFile) {
        if (!this.traceability)
            return content;
        // Match "word:ID[]" patterns within [item] block content
        // ID format: letters + digits, optionally followed by hyphens/dots + more alphanumeric
        const relRegex = /\b(\w+):([A-Za-z]\d*(?:[-.]\w+)*?)\[\]/g;
