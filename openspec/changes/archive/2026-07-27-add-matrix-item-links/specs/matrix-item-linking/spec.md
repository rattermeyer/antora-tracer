...
The system SHALL generate different link paths based on the execution context (Antora build vs. CLI).
...
- **THEN** links use `../../pages/` prefix to navigate from `attachments/traceability/` to pages directory

---

### Requirement: Source file paths are normalized
The system SHALL normalize item `sourceFile` paths to be relative to the `pages/` directory at parse time.
