...
### Requirement: Playbook refs exist on the remote
The script SHALL verify that every branch and tag referenced by the CI playbook content sources exists on the remote repository.

#### Scenario: playbook references a missing ref
- **WHEN** the playbook references branch `v0.20.x` but the remote lacks it
