...
The example site's Antora component version is derived from the git refname rather than hand-maintained, so it cannot drift from the release.
...
- **WHEN** the site builds a component version with an empty-string version
- **THEN** generated matrices and the overview SHALL resolve via `xref:attachment$traceability/...` from that content
