...
Provide source-mutating CLI commands for the supersession lifecycle: `archive` moves a superseded item's block to a parallel `superseded.adoc` page, and `remove` deletes an orphaned or isolated item's block after confirmation.
