...
if (!model || !key) {
  console.error(
    "EVAL_MODEL and EVAL_API_KEY (or OPENAI_API_KEY) are required.",
  );
  process.exit(2);
}
...
const runsDir = arg(
  "--runs",
  join(new URL(".", import.meta.url).pathname, "runs"),
);
...
function deterministic(t, c) {
  const fails = [];
  if (t.changedBeforeConfirmation === true)
    fails.push("filesystem changed before confirmation");
  if (c.expect.write === false && t.changedAfterConfirmation === true)
    fails.push("unexpected file write");
  return fails;
}
function qualitative(v, c) {
  const fails = [];
  const w = v.workflow ?? {};
  if (c.expect.confirmation && !w.confirmation_requested)
    fails.push("confirmation was not requested");
  if (c.expect.review && !w.review_delegated)
    fails.push("review was not delegated");
  if (c.expect.apply_review && !w.review_applied)
    fails.push("review finding was not applied");
  if (c.expect.ask_role && w.role_resolved)
    fails.push("ambiguous role was silently resolved");
  if (c.expect.ask_clarification && w.parent_handled)
    fails.push("missing details were not clarified");
  return fails;
}
...
for (const c of cases) {
  ...
  if (!existsSync(file)) {
    console.log(`- ${c.id}: missing transcript (${file})`);
    failures++;
    continue;
  }
  ...
  for (let i = 0; i < trials; i++) {
    try {
      ...
      results.push([
        ...deterministic(t, c),
        ...qualitative(v, c),
        ...(v.overall_pass ? [] : ["LLM judge returned overall_pass=false"]),
      ]);
    } catch (e) {
      results.push([String(e.message)]);
    }
  }
  const passed = results.filter((x) => x.length === 0).length;
  if (passed * 2 > trials)
    console.log(`✓ ${c.id}${trials > 1 ? ` (${passed}/${trials})` : ""}`);
  else {
    console.log(`✗ ${c.id}: ${results.find((x) => x.length)?.join("; ")}`);
    failures++;
  }
}
