...
`;
  ...
  describe('Matrix with Coverage Relations', () => {
    it('should handle reverse-direction relationships (column → row)', () => {
      ...
      graph.addItem({ id: 'REQ-001', role: 'requirement', title: 'User Auth', attributes: {}, sourceFile: 'test.adoc' });
      graph.addItem({ id: 'REQ-002', role: 'requirement', title: 'Password Rules', attributes: {}, sourceFile: 'test.adoc' });

      // Column items (architecture and test)
      graph.addItem({ id: 'ARC-001', role: 'architecture', title: 'Auth Architecture', attributes: {}, sourceFile: 'test.adoc' });
      graph.addItem({ id: 'TST-001', role: 'test', title: 'Auth Tests', attributes: {}, sourceFile: 'test.adoc' });

      // Relationships are column→row (reverse direction):
      graph.addRelationship({ id: 'REL-001', fromId: 'ARC-001', targetId: 'REQ-001', type: 'addresses', sourceFile: 'test.adoc' });
      graph.addRelationship({ id: 'REL-002', fromId: 'TST-001', targetId: 'REQ-001', type: 'verifies', sourceFile: 'test.adoc' });
...
`);
    ...
    it('should handle forward-direction relationships (row → column)', () => {
      ...
      const graph = new TraceabilityGraph();
      graph.addItem({ id: 'IMP-001', role: 'implementation', title: 'Auth Service', attributes: {}, sourceFile: 'test.adoc' });
      graph.addItem({ id: 'IMP-002', role: 'implementation', title: 'Password Service', attributes: {}, sourceFile: 'test.adoc' });
      graph.addItem({ id: 'REQ-001', role: 'requirement', title: 'User Auth', attributes: {}, sourceFile: 'test.adoc' });
      graph.addItem({ id: 'REQ-002', role: 'requirement', title: 'Password Rules', attributes: {}, sourceFile: 'test.adoc' });

      graph.addRelationship({ id: 'REL-001', fromId: 'IMP-001', targetId: 'REQ-001', type: 'satisfies', sourceFile: 'test.adoc' });
      graph.addRelationship({ id: 'REL-002', fromId: 'IMP-002', targetId: 'REQ-002', type: 'satisfies', sourceFile: 'test.adoc' });
