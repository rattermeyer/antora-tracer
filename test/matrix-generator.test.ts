...
`,
  ...
  describe("Module-Aware Links", () => {
    ...
    it("should handle subdirectory sourceFile with ROOT module (no prefix)", () => {
      const resolver = new LinkResolver({ relativePathPrefix: "../../" });
      ...
      const link = resolver.generateItemLink(item);
      expect(link).to.equal("../../traceability/index.html#REQ-035");
    });

    it("should handle subdirectory sourceFile with non-ROOT module", () => {
      const resolver = new LinkResolver({ relativePathPrefix: "../../" });
