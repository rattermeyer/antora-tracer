...
function createItem(id: string, role: string, title?: string): any {
  return {
    id,
    role,
    title: title || id,
...
function processContent(
  extension: RequirementsTraceabilityExtension,
  content: string,
  file = "input.adoc",
) {
...
describe("TraceabilityGraph - Extended Queries", () => {
  ...
  describe("getRelatedItems()", () => {
    ...
    it("should filter by relation type", () => {
      ...
      graph.addRelationship(
        createRel("R1", "IMP-001", "DES-001", "implements"),
      );
  ...
  describe("toDot()", () => {
    ...
    it("should return empty string for unknown item", () => {
      expect(graph.toDot("NONEXISTENT")).to.equal("");
    });

    it("should return a digraph DOT string", () => {
      const dot = graph.toDot("REQ-001");
      expect(dot).to.be.a("string");
      expect(dot).to.include("digraph Traceability");
      expect(dot).to.include("rankdir=LR");
    });

    it("should include the source node", () => {
      const dot = graph.toDot("REQ-001");
      expect(dot).to.include('"REQ-001"');
    });

    it("should include directly related nodes at depth 1", () => {
      const dot = graph.toDot("REQ-001");
      // DES-001 has an addresses relationship to REQ-001 (incoming)
      // toDot now traverses both outgoing and incoming relationships
      expect(dot).to.include('"REQ-001"');
      expect(dot).to.include('"DES-001"');
    });

    it("should include nodes connected via incoming relationships", () => {
      // REQ-001 has no outgoing relationships, but DES-001 addresses it
      // The bidirectional traversal should find DES-001 via reverse index
      const dot = graph.toDot("REQ-001");
      expect(dot).to.include('"DES-001"');
      expect(dot).to.include('"REQ-001"');
    });

    it("should limit to depth 1 by default", () => {
      const dot = graph.toDot("IMP-001"); // IMP→DES→REQ (2 hops)
      // IMP-001 outgoing -> DES-001; DES-001 outgoing -> REQ-001
      // At depth 1: IMP-001 and DES-001
      expect(dot).to.include('"IMP-001"');
      expect(dot).to.include('"DES-001"');
      expect(dot).to.not.include('"REQ-001"');
    });

    it("should include deeper nodes with depth > 1", () => {
      const dot = graph.toDot("IMP-001", 2);
      expect(dot).to.include('"IMP-001"');
      expect(dot).to.include('"DES-001"');
      expect(dot).to.include('"REQ-001"');
    });

    it("should color nodes by role using ROLE_COLORS", () => {
      const dot = graph.toDot("DES-001");
      // Design role has a color in ROLE_COLORS
      expect(dot).to.include("fillcolor=");
      expect(dot).to.include("fontcolor=white");
    });

    it("should label edges with relation type", () => {
      const dot = graph.toDot("DES-001");
      expect(dot).to.include('label="addresses"');
    });

    it("should use dir=both for bidirectional edges", () => {
      // Add a bidirectional relationship for REQ-001
      const bidirRel = createRel("R3", "REQ-001", "DES-001", "leads_to");
      bidirRel.bidirectional = true;
      graph.addRelationship(bidirRel);

      const dot = graph.toDot("REQ-001");
      expect(dot).to.include("dir=both");
  ...
  describe("toVegaLite()", () => {
    ...
    it("should return a JSON string", () => {
      const spec = graph.toVegaLite();
      expect(spec).to.be.a("string");
      const parsed = JSON.parse(spec);
      expect(parsed).to.be.an("object");
    });

    it("should return a valid Vega-Lite spec (global)", () => {
      const spec = graph.toVegaLite();
      const parsed = JSON.parse(spec);
      expect(parsed.$schema).to.include("vega-lite");
      expect(parsed.mark).to.equal("bar");
      expect(parsed.encoding).to.be.an("object");
    });

    it("should include global coverage data by role", () => {
      const spec = graph.toVegaLite();
      const parsed = JSON.parse(spec);
      // Should have data with role-based counts
      expect(parsed.data).to.be.an("object");
      expect(parsed.data.values).to.be.an("array");
      expect(parsed.data.values.length).to.be.greaterThan(0);
    });

    it("should return per-item coverage when itemId is provided", () => {
      const spec = graph.toVegaLite("DES-001");
      const parsed = JSON.parse(spec);
      expect(parsed.$schema).to.include("vega-lite");
      expect(parsed.mark).to.equal("bar");
    });
    ...
    it("should return empty string for unknown item", () => {
      expect(graph.toVegaLite("NONEXISTENT")).to.equal("");
    });

    it("should show different data for per-item vs global", () => {
      const globalSpec = graph.toVegaLite();
      const itemSpec = graph.toVegaLite("DES-001");
  ...
  describe("validate() with circular references", () => {
    ...
    it("should not report bidirectional pair as circular reference", () => {
      const graph = new TraceabilityGraph();
      graph.addItem(createItem("REQ-001", "requirement", "Login"));
      graph.addItem(createItem("REQ-002", "requirement", "Auth"));
      // Manually mark as bidirectional to simulate merged pair
      const rel = createRel("R1", "REQ-001", "REQ-002", "addresses");
  ...
  describe("bidirectional relationship merge", () => {
    ...
    it("should store single-direction relationship normally", () => {
      const graph = new TraceabilityGraph();
      ...
      const allRels = graph.getAllRelationships();
      expect(allRels).to.have.lengthOf(1);
      expect(allRels[0].bidirectional).to.not.be.true;
    });

    it("should not merge when inverse type is not configured", () => {
      const graph = new TraceabilityGraph();
      ...
      const allRels = graph.getAllRelationships();
      expect(allRels).to.have.lengthOf(2);
    });

    it("should not merge same-type pairs (intentional cycles)", () => {
      const graph = new TraceabilityGraph();
      graph.addItem(createItem("A", "requirement", "Item A"));
...
describe("RequirementsTraceabilityExtension - API Methods", () => {
  ...
  describe("getRelationships() with type filter", () => {
    it("should filter relationships by type", () => {
      ...
      const implementsRels = extension.getRelationships(
        "IMP-001",
        "implements",
      );
