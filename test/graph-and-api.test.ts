...
describe("TraceabilityGraph - Extended Queries", () => {
  ...
  describe("toDot()", () => {
    ...
    it("should include directly related nodes at depth 1", () => {
      const dot = graph.toDot("REQ-001");
      // DES-001 has an addresses relationship to REQ-001 (reverse direction)
      // getRelationships from REQ-001 is empty, but reverse gives DES-001
      // The BFS starts from REQ-001 and looks at outgoing, so it won't see
      // DES-001. Let's test from DES-001 which has outgoing relationships.
      const dotFromDes = graph.toDot("DES-001");
      expect(dotFromDes).to.include('"DES-001"');
      expect(dotFromDes).to.include('"REQ-001"');
    });
