...
describe('Traceability Graph', function() {
  ...
  describe('Graph Structure', function() {
    it('should initialize with empty maps', function() {
      ...
      expect(extension.graph.getAllRequirements()).to.be.an('array').with.lengthOf(0);
      expect(extension.graph.getAllImplementations()).to.be.an('array').with.lengthOf(0);
      expect(extension.graph.getAllTests()).to.be.an('array').with.lengthOf(0);
    });
