...
function substituteRelationshipLinks(
  content: string,
  currentFile: string,
  ...
  return content.replace(
    relRegex,
    (_match: string, _relType: string, targetId: string) => {
...
describe("Relationship Link Substitution", () => {
  describe("Regex matching", () => {
    it("should match addresses:REQ-001[] pattern", () => {
      ...
      graph.addItem(
        createItem("REQ-001", "requirement", "Req 1", "requirements.adoc"),
      );

      const content = "addresses:REQ-001[]";
      const result = substituteRelationshipLinks(
        content,
        "architecture.adoc",
        graph,
      );
      expect(result).to.equal(
        "addresses: xref:requirements.adoc#REQ-001[REQ-001]",
      );
    });
    ...
    it("should match verifies:TEST-001[] pattern", () => {
      ...
      expect(result).to.equal(
        "verifies: xref:test-plan.adoc#TEST-001[TEST-001]",
      );
    });
    ...
    it("should match multiple relationship macros in one line", () => {
      ...
      graph.addItem(
        createItem("REQ-002", "requirement", "Req 2", "requirements.adoc"),
      );
  ...
  describe("Edge cases", () => {
    it("should handle IDs with hyphens and dots", () => {
      ...
      expect(result).to.equal(
        "addresses: xref:requirements.adoc#SYS.SEC-001[SYS.SEC-001]",
      );
    });
    ...
    it("should preserve surrounding text", () => {
      ...
      const content =
        "Auth service that addresses:REQ-001[] and satisfies:REQ-001[].";
      const result = substituteRelationshipLinks(
        content,
        "architecture.adoc",
        graph,
      ...
      expect(result).to.include(
        "satisfies: xref:requirements.adoc#REQ-001[REQ-001]",
      );
      expect(result).to.equal(
        "Auth service that addresses: xref:requirements.adoc#REQ-001[REQ-001] and satisfies: xref:requirements.adoc#REQ-001[REQ-001].",
      );
    });
  });
  ...
  describe("Verbatim block preservation", () => {
    ...
    function substitutePreservingVerbatim(
      content: string,
      currentFile: string,
      graph: TraceabilityGraph,
    ): string {
      ...
      if (ranges.length === 0) return processSegment(content);
    ...
    it("should preserve inline macros inside [source,asciidoc] block", () => {
      ...
      const result = substitutePreservingVerbatim(
        content,
        "architecture.adoc",
        graph,
      );
