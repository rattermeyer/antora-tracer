export async function runProjectsCommand(args: string[]): Promise<void> {
  ...
  const endpoint =
    flags.endpoint ?? process.env.ID_SERVER_ENDPOINT ?? "http://localhost:8080";
  const adminToken = flags["admin-token"] ?? process.env.ID_SERVER_ADMIN_TOKEN;
  if (adminToken === undefined) {
    throw new Error(
      "admin token required: pass --admin-token or set ID_SERVER_ADMIN_TOKEN",
    );
  }
  ...
  switch (subcommand) {
    case "list": {
      ...
      await ensureOk(res);
      const tenants = (await res.json()) as string[];
    ...
    default:
      throw new Error(
        `unknown subcommand "${subcommand ?? ""}" (expected list, add, update, or remove)`,
      );
  }
}
