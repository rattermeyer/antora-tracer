 ...
 */
function listRefs(cwd: string): { branches: string[]; tags: string[] } {
 ...
 */
export function gatherState(cwd: string): ReleaseState {
  ...
  for (const src of playbook.content?.sources ?? []) {
    ...
    for (const t of src.tags ?? []) refs.push(t);
  }
 ...
 */
export function checkConsistency(state: ReleaseState): string[] {
  ...
  if (!branches.includes(maintBranch)) {
    findings.push(
      `missing maintenance branch ${maintBranch} for release ${version}`,
    );
  }
  ...
  for (const ref of playbookRefs) {
    if (ref === "HEAD") continue; // local preview sentinel, not a real ref
