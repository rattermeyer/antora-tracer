...
program
  ...
  .action(async (id: string, options: any) => {
    ...
    if (!graph.isSuperseded(id)) {
      console.error(
        chalk.red(
          `Item ${id} is not superseded — only superseded items can be archived.`,
        ),
      );
      process.exit(1);
    }
    ...
    if (!extracted) {
      console.error(
        chalk.red(
          `Could not locate the block for ${id} in ${item.sourceFile}:${item.sourceLine ?? "?"}. No change made.`,
        ),
      );
      process.exit(1);
    }
    ...
    const lines = content.split("\n");
    lines.splice(
      extracted.startLine - 1,
      extracted.endLine - extracted.startLine + 1,
    );
...
program
  ...
  .action(async (id: string, options: any) => {
    ...
    const isolated =
      graph.getRelationships(id).length === 0 &&
    ...
    if (!extracted) {
      console.error(
        chalk.red(`Could not locate the block for ${id}. No change made.`),
      );
      process.exit(1);
    }
    ...
    if (orphaned) {
      ...
      const answer = await promptLine(
        `This item is NOT superseded. Type the ID to confirm removal: `,
      );
      if (answer !== id) {
