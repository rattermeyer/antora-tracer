...
A `use_case` item describes a user goal in Karl Wiegers tabular form.

== Traceability

A `use_case` is a traceable item.
Check the traceability configuration if and to what role it should be linke upwards.
If not existent, think about writing downstream items.

Do not invent role or relation names — take them from the config.
If the project defines no use-case role or incoming or outgoing relations, tell the user use cases are not wired into their traceability model yet.
...
```bash
antora-tracer next-id --prefix <use_case_prefix> -i <docs-dir>
```
...
[asciidoc]
----
[#<use-case-id>, item, role=use_case, title="<Actor> <action verb> <observable outcome>"]
--
.Use Case: <title>
...
The `<traceability-relations>` relation names the requirements this use case is derived from (if defined in the traceability model).
Only include it if the relation is defined in `traceability.yml` — see the discovery step.
...
=== Structure
- [ ] Uses `role=use_case` with padded `<use-case-prefix>-id` ID
...
=== Traceability
- [ ] At least one traceability relationship
- [ ] All referenced requirement IDs exist in the requirements document
- [ ] `traceability:links[]` is present
