...
export class TraceabilityGraph {
   ...
   */
  private _isPrimaryType(type: string): boolean {
    const inverseLabels = this._configLoader?.getConfig().inverseLabels;
    if (inverseLabels && type in inverseLabels) return true;
   ...
   */
  private _removeFromIndex(relationship: ItemRelationship): void {
   ...
   */
  private _resolveInverseType(type: string): string | undefined {
    const inverseLabels = this._configLoader?.getConfig().inverseLabels;
    // Forward lookup: type is the primary, return its inverse
    if (inverseLabels?.[type]) return inverseLabels[type];
    // Reverse lookup: type is the inverse, find its primary
    if (inverseLabels) {
      for (const [primary, inverse] of Object.entries(inverseLabels)) {
        if (inverse === type) return primary;
      }
    }
    const inv = INVERSE_MAP[type as keyof typeof INVERSE_MAP];
    if (inv) return inv;
    ...
    return PRIMARY_MAP[type as keyof typeof PRIMARY_MAP];
  }
