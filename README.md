...
1. Install
   ...
   ```bash
   npm install @antora-tracer/core --save-dev
   ```
2. Add to `antora.yml`
...
pi install npm:@antora-tracer/core

# pi — or, with a regular npm install, add to .pi/settings.json:
#   { "skills": ["../node_modules/@antora-tracer/core/skills"] }

# Claude Code
cp -r node_modules/@antora-tracer/core/skills/* ~/.claude/skills/

# OpenAI Codex
cp -r node_modules/@antora-tracer/core/skills/* ~/.codex/skills/
```
...
## Reimplementation Experiment

Can a complete, working tool be rebuilt from its requirements specification alone — no design documentation, no architecture, no source?

This repository's example site captures the tool's requirements as traceability items (`examples/tracer/modules/requirements/`). A sibling repository, [antora-tracer-reimplementation](https://github.com/rattermeyer/antora-tracer-reimplementation), turns those requirements into a reproducible experiment:

- **`requirements/spec.adoc`** — the specification (constraints + functional + quality), extracted from the example site; build tooling excluded, tool name removed.
- **`golden/`** — the oracle: matrices generated by the published `@antora-tracer/core` npm package, never from source.
- **`harness/`** — clone + strip + golden generation + content-level comparison.

The experiment asks: given only the requirements, does an LLM produce a tool that generates the same matrices?

### Kick-off prompt

Clone the sibling repository and hand this prompt to an LLM (or run an agent in that repo):

```text
You are implementing a requirements traceability extension for Antora, from a formal requirements specification. You are building from the specification alone — not extending or consulting any existing implementation.

## Task

Implement, from scratch, a complete, runnable tool that satisfies every requirement in the specification.

## Specification

Read `requirements/spec.adoc`. It is the complete and authoritative specification, in three parts:

- **Constraints** (`CON-*`) — non-negotiable boundaries that bound every implementation decision.
- **Functional Requirements** (`REQ-*`) — what the tool must do.
- **Quality Requirements** (`QA-*`) — quality levels the tool must meet.

Implement all of it. Do not invent features that are not described. Do not skip any requirement.

## Validation target

Your tool must process the AsciiDoc documentation site in `fixtures/antora-tracer/examples/` and produce traceability matrices whose content matches the reference output. The site declares traceability items with the `[#ID, item, role=..., title="..."]` block macro, links them with inline relationship macros, and is configured by `examples/traceability.yml`.

## Deliverable

A complete, runnable project in the `implementation/` directory:

- the Antora extension
- a command-line interface (see the CLI requirements)
- build configuration
...
Your tool must generate, for each configured matrix, a CSV file named `matrix-<name>.csv`:

- header row: `Row ID,Row Title,<column-role>...`
- one row per row-item; first column = the row item ID
- each cell: empty when the row item has no relationship to that column role, otherwise the covering item's ID

The validator is `harness/compare.mjs`.

## Ground rules

- Implement from the specification only. The fixtures contain the example site; its `src/`, `lib/`, `test/`, and `openspec/` directories have been removed. Do not fetch or copy any reference implementation.
- Do not read the `golden/` directory — it contains the expected output.
- TypeScript, Node.js 20+, ESM (per the specification).
- You may consult Antora's public extension API documentation.

## Definition of done

`node harness/compare.mjs implementation/matrices/` reports PASS for every matrix.
```
