...
let USER_HOME = '';
try {
  LIB_DIR = new URL('.', (typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('index.cjs', document.baseURI).href))).pathname.replace(/\/$/, '');
  ROOT_DIR = new URL('../../', (typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('index.cjs', document.baseURI).href))).pathname.replace(/\/$/, '');
  DATA_DIR = new URL('../../data', (typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('index.cjs', document.baseURI).href))).pathname;
...
async function _requireFsp$1() {
  ...
  try {
    _fsp$1 = require('node:fs/promises');
    _fsConstants$1 = (require('node:fs')).constants;
  } catch {
    _fsp$1 = null;
  }
}
  ...
  async create(backend, opts = {}) {
    ...
    if (converter) {
      ...
      if (
        ...
        const { TemplateConverter } = await Promise.resolve().then(function () { return template; });
...
async function _requireFsp() {
  ...
  try {
    _fsp = require('node:fs/promises');
    _fsConstants = (require('node:fs')).constants;
  } catch {
    _fsp = null;
  }
}
 ...
 * @returns {Promise<Document>} A Promise that resolves to the Document.
 */
async function loadFile$1(filename, options = {}) {
...
async function convert(input, options = {}) {
  ...
  if (siblingPath) {
    ...
    if (mkdirs) {
      ...
      if (!(await _isDirectory$1(outdir))) {
    ...
    if (
      ...
      if (
        ...
        if (mkdirs) {
          ...
          if (!(await _isDirectory$1(stylesoutdir))) {
            throw new Error(
        ...
        if (copyAsciidoctorStylesheet) {
          ...
          if (stylesheetSrc !== stylesheetDest) {
            const warnOnFailure = !(await _isFile$1(stylesheetDest));
            ...
            if (stylesheetData) {
              ...
              if (
                ...
                !(await _isDirectory$1(stylesheetOutdir))
              ) {
                if (!mkdirs) {
                  throw new Error(
 ...
 * @returns {Promise<import('./document.js').Document|string>} the Document if output was written to a file, otherwise the converted String
 */
async function convertFile(filename, options = {}) {
 ...
 * @returns {Promise<boolean>}
 */
async function _isDirectory$1(dir) {
  try {
    const { stat } = require('node:fs/promises');
...
async function loadFile(filename, options = {}) {
  return loadFile$1(filename, options)
}
  ...
  async _scanDir(templateDir, fileExtFilter, templateCache = null) {
    ...
    for (const basename of entries) {
      ...
      if (ext === 'nunjucks' || ext === 'njk') {
        ...
        const compiled = nunjucks.compile(await node_fs.promises.readFile(file, 'utf8'), env);
        ...
        const renderFn = env.compile(await node_fs.promises.readFile(file, 'utf8'), hbsOpts);
        ...
        const renderFn = ejs.compile(await node_fs.promises.readFile(file, 'utf8'), ejsOpts);
        template = { render: renderFn, file };
      } else if (ext === 'pug') {
...
async function _isDirectory(p) {
  try {
    return (await node_fs.promises.stat(p)).isDirectory()
  } catch {
    return false
  }
}
...
async function _isFile(p) {
  try {
    return (await node_fs.promises.stat(p)).isFile()
  } catch {
    return false
  }
}
...
const template = /*#__PURE__*/Object.freeze({
  __proto__: null,
...
exports.Block = Block;
exports.BlockMacroProcessor = BlockMacroProcessor;
exports.BlockProcessor = BlockProcessor;
exports.ContentModel = ContentModel;
exports.ConverterBase = ConverterBase;
exports.ConverterCustomFactory = CustomFactory$1;
exports.ConverterFactory = Converter;
exports.Cursor = Cursor;
exports.DefaultConverterFactory = DefaultFactory$1;
exports.DefaultSyntaxHighlighterFactory = DefaultFactory;
exports.DocinfoProcessor = DocinfoProcessor;
...
exports.PreprocessorReader = PreprocessorReader;
exports.Processor = Processor;
exports.ProcessorExtension = ProcessorExtension;
...
exports.SyntaxHighlighterBase = SyntaxHighlighterBase;
exports.Timings = Timings;
exports.TreeProcessor = TreeProcessor;
exports.convert = convert;
exports.convertFile = convertFile;
exports.deriveBackendTraits = deriveBackendTraits;
